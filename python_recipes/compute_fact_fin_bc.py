# -*- coding: utf-8 -*-
"""
Recipe: fact_fin_bc   (FINISHING / 2nd Step — mirror of fact_conf_bc)
Inputs : fin_bc_grq2, dim_operator, meta_cq_descriptions (TBM6)
Output : fact_fin_bc
         One row per 2nd-Step Before-Cure CQ event, enriched with operator + CQ description.

>>> If your finishing before-cure source is named differently, change SRC below.
"""

import dataiku
import pandas as pd

SRC = "fin_bc_grq2"        # <-- finishing before-cure CQ source dataset

bc  = dataiku.Dataset(SRC).get_dataframe()
...
print(f"fact_fin_bc rows: {len(fact):,}")
dataiku.Dataset("fact_fin_bc").write_with_schema(fact)
