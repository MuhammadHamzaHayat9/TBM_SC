# -*- coding: utf-8 -*-
"""
Tire Building Scorecard — Dash Webapp
Filters: BU, Crew, Week, Operator (click on leaderboard name to drill in)

This file is intended to be pasted into the Python tab of a Dataiku Code
Webapp (Dash). Dataiku provides the `app` variable already.
"""
...
# ============================================================
# 1. LOAD FACT TABLES (operator-grain so all filters work)
# ============================================================
def load(name):
...
TOP   = load("agg_top_performers")  # for the leaderboard score

# Normalize OP_ID to string everywhere
def norm_op(x):
    if pd.isna(x): return None
    try:    return str(int(float(x))).zfill(6)
    except: return str(x).strip().zfill(6)

for df in (PROD, BC, AC, SCRAP, TOP):
...
def filter_facts(bus, crews, weeks, op_id):
    """Return filtered copies of all 4 fact tables."""
    def f(df):
        d = df
        ...
        return d
    return f(PROD), f(BC), f(AC), f(SCRAP)
...
def kpi_tile(label, value, suffix="", color=None):
    ...
    return html.Div(style=CARD, children=[
        ...
        html.Div(f"{value}{suffix}", style={"color": color, "fontSize": "36px",
...
app.layout = html.Div(style={"backgroundColor": COLORS["bg"], "padding": "16px",
    ...
    html.Div(style={"display": "grid", "gridTemplateColumns": "repeat(3, 1fr)",
        ...
        html.Div(style=CARD, children=[
            html.H4("NC Scrap — TBM Breakdown", style={"margin": "0 0 8px 0"}),
            dcc.Graph(id="donut-scrap", config={"displayModeBar": False}),
        ]),
    ]),
...
def update_all(bus, crews, weeks, op_id):
    prod, bc, ac, scrap = filter_facts(bus, crews, weeks, op_id)
    ...
    # ---------- Donuts ----------
    def make_donut(df, label_col, color_total):
        d = df.copy()
        if label_col == "CQ_DESCRIPTION":
            d = d.dropna(subset=["CQ_CODE_STR"])
            d[label_col] = d[label_col].fillna(d["CQ_CODE_STR"])
            grp = d.groupby(label_col).size().sort_values(ascending=False).head(10)
        else:  # TBM for scrap
            grp = d.groupby(label_col)["OP_SCRAP_LBS_BY_TIRES"].sum() \
    ...
    donut_bc    = make_donut(bc,    "CQ_DESCRIPTION", bc_cnt)
    donut_ac    = make_donut(ac,    "CQ_DESCRIPTION", ac_cnt)
    donut_scrap = make_donut(scrap, "TBM",            scrap_lb)
    ...
    sc_w = (scrap.groupby(keys, dropna=False)["OP_SCRAP_LBS_BY_TIRES"]
                 .sum().reset_index(name="SCRAP_LBS"))

    t = p_w.merge(bc_w, on=keys, how="left") \
           .merge(ac_w, on=keys, how="left") \
           .merge(sc_w, on=keys, how="left")
    for c in ["BC_COUNT","AC_COUNT","SCRAP_LBS"]:
        t[c] = t[c].fillna(0)
    ...
    trend_fig.add_bar(x=t["WEEK_LABEL"], y=t["TIRES_BUILT"],
                      name="Tires Built", marker_color=COLORS["primary"], opacity=0.55,
                      yaxis="y1")
    ...
    # ---------- Top performers (filter out blank operators) ----------
    ...
    table_rows = [html.Tr(
        [html.Th(c, style={"padding": "6px", "textAlign": "left"})
         for c in ["#", "Operator", "BU", "Crew", "Tires", "Score"]],
        style={"backgroundColor": COLORS["primary"], "color": "white"})]
    ...
    return kpis, donut_bc, donut_ac, donut_scrap, trend_fig, top_table
